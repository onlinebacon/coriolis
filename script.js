...
const set2DTransform = () =>
	ctx.setTransform(
		(radiusRatio * canvasSize) / 2,
		0,
		0,
		(-radiusRatio * canvasSize) / 2,
		canvasSize * 0.5,
...
const clonePoint = ({ color, pos, path, fixed }) => ({
	color,
	pos: pos.clone(),
	path,
	fixed,
...
const createInertialPath = (a, b, TRAVEL_TIME, subdivisions) => {
	...
	return convertToInertial(createPath(a, b, subdivisions), TRAVEL_TIME)
}
...
const parseCoord = (text) => {
	const [lat, lon] = text.split(/\s*,\s*/).map(Number)
...
function reset() {
	const coord_a = parseCoord(startInput.value)
	const coord_b = parseCoord(endInput.value)

	if (!coord_a || !coord_b) return
	...
	aPoint = addPoint(...coord_a, '#fff')
	bPoint = addPoint(...coord_b, '#fff')

	groundPath = createPath(aPoint, bPoint, 6)
	inertialPath = createInertialPath(aPoint, bPoint, TRAVEL_TIME, 6)
	...
	target1 = addPoint(0, 0, '#f20')
	target2 = addPoint(0, 0, '#07f')
...
startInput.oninput = reset
endInput.oninput = reset
